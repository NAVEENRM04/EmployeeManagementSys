...
//Admin
  ...
  const AdminUserDetail= ()=> axios.get(`http://localhost:8080/api/admin/getdata`,{headers})
//Manager
  const ManagerUserDetail = async() => await axios.get("http://localhost:8080/api/management/GetActiveUserForManager",{headers});
  const MbanUserB =(userName,isactive)=>axios.put(`http://localhost:8080/api/manager/updateActiveStatus`,{userName:userName,isactive:isactive},{headers});
...
export {userRegister,userLogin,checkCode,getCode,changePassword,getUserDetail,updateProfilepic,addImage,userDetail,TdeleteUser,banUserB,AdminUserDetail,ManagerUserDetail,MbanUserB};